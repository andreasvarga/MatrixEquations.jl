...
ar = rand(Ty,n,n); ar = ar-2*norm(ar)*Matrix(I,n,n);
ars = Symmetric(ar); ars = ars-2*norm(ars)*Matrix(I,n,n);
ard = -Diagonal(rand(Ty,n))
br = rand(Ty,n,m);
ac = rand(Ty,n,n)+im*rand(Ty,n,n); ac = ac-2*norm(ac)*Matrix(I,n,n);
ach = Hermitian(ac); ach = ach-2*norm(ach)*Matrix(I,n,n);
acd = -Diagonal(rand(Ty,n)+im*rand(Ty,n))
...
@time u = plyapc(ars,br);
x = u*u'; @test norm(ars*x+x*ars+br*br')/norm(x)/norm(ars) < reltol

@time u = plyapc(ard,br);
x = u*u'; @test norm(ard*x+x*ard+br*br')/norm(x)/norm(ard) < reltol
...
@time u = plyapc(ach,bc);
x = u*u'; @test norm(ach*x+x*ach'+bc*bc')/norm(x)/norm(ach) < reltol

@time u = plyapc(acd,bc);
x = u*u'; @test norm(acd*x+x*acd'+bc*bc')/norm(x)/norm(acd) < reltol
...
@time u = plyapc(ach',cr');
x = u'*u; @test norm(ach'*x+x*ach+cr'*cr)/norm(x)/norm(ach) < reltol

@time u = plyapc(acd',cr');
x = u'*u; @test norm(acd'*x+x*acd+cr'*cr)/norm(x)/norm(acd) < reltol
...
ar = rand(Ty,n,n); ar = ar-2*norm(ar)*Matrix(I,n,n); er = rand(n,n); ar = er*ar;
...
@time u = real(plyapc(complex(ar),complex(er),complex(br)));
x = u*u'; @test norm(ar*x*er'+er*x*ar'+br*br')/norm(x)/norm(ar) < reltol
...
if Ty <: BlasFloat
@time u = plyapc(ar,er,br; blocksize = 10);
x = u*u'; @test norm(ar*x*er'+er*x*ar'+br*br')/norm(x)/norm(ar) < reltol

@time u = plyapc(ar',er',cr'; blocksize = 10);
x = u'*u; @test norm(ar'*x*er+er'*x*ar+cr'*cr)/norm(x)/norm(ar) < reltol

@time u = plyapc(ac,ec,bc; blocksize = 10);
x = u*u'; @test norm(ac*x*ec'+ec*x*ac'+bc*bc')/norm(x)/norm(ac) < reltol

@time u = plyapc(ac',ec',cc'; blocksize = 10);
x = u'*u; @test norm(ac'*x*ec+ec'*x*ac+cc'*cc)/norm(x)/norm(ac) < reltol  
end
end
end
...
U = copy(R)                
MatrixEquations.pglyap2!(A, E, U, β, α, adj = true, disc = false)
...
U = copy(R)
MatrixEquations.pglyap2!(A, E, U, β, α, adj = false, disc = false)
X = U*U'; @test norm(A*X*E'+E*X*A'+R*R')/max(1,norm(X))/norm(A) < reltol &&
...
MatrixEquations.pglyap2!(A1, E1, U, β, α, adj = false, disc = false)
...
@time u = plyaps(arsd,br);
x = u*u'; @test norm(arsd*x+x*arsd+br*br')/norm(x)/norm(arsd) < reltol

@time u = plyaps(ard,br);
x = u*u'; @test norm(ard*x+x*ard+br*br')/norm(x)/norm(ard) < reltol
...
@time u = plyaps(arsd',cr');
x = u'*u; @test norm(arsd'*x+x*arsd+cr'*cr)/norm(x)/norm(arsd) < reltol

@time u = plyaps(ard',cr');
x = u'*u; @test norm(ard*x+x*ard+cr'*cr)/norm(x)/norm(ard) < reltol
...
x = u*u'; @test norm(acs*x+x*acs'+bc*bc')/norm(x)/norm(acs) < reltol

@time u = plyaps(acsd,bc);
x = u*u'; @test norm(acsd*x+x*acsd'+bc*bc')/norm(x)/norm(acsd) < reltol

@time u = plyaps(acd,bc);
x = u*u'; @test norm(acd*x+x*acd'+bc*bc')/norm(x)/norm(acd) < reltol

@time u = plyaps(acs',cc');
x = u'*u; @test norm(acs'*x+x*acs+cc'*cc)/norm(x)/norm(acs) < reltol

@time u = plyaps(acsd',cc');
x = u'*u; @test norm(acsd'*x+x*acsd+cc'*cc)/norm(x)/norm(acsd) < reltol

@time u = plyaps(acd',cc');
x = u'*u; @test norm(acd'*x+x*acd+cc'*cc)/norm(x)/norm(acd) < reltol
...
if Ty <: BlasFloat
@time u = plyaps(as,es,br,blocksize = 10);
x = u*u'; @test norm(as*x*es'+es*x*as'+br*br')/norm(x)/norm(as) < reltol

@time u = plyaps(as',es',cr',blocksize = 10);
x = u'*u; @test norm(as'*x*es+es'*x*as+cr'*cr)/norm(x)/norm(as) < reltol

@time u = plyaps(acs,ecs,bc,blocksize = 10);
x = u*u'; @test norm(acs*x*ecs'+ecs*x*acs'+bc*bc')/norm(x)/norm(as) < reltol

@time u = plyaps(acs',ecs',cc',blocksize = 10);
x = u'*u; @test norm(acs'*x*ecs+ecs'*x*acs+cc'*cc)/norm(x)/norm(as) < reltol
end
...
F = UpperTriangular(rand(Ty,n,n))
R = copy(F)
@time plyapcs!(as,es,R,adj = true,blocksize = 10);
x = R'*R; @test norm(as'*x*es+es'*x*as+F'*F)/norm(x)/norm(as) < reltol  #fails

F = UpperTriangular(rand(Ty,n,n))
R = copy(F)
@time plyapcs!(as,es,R,adj = false,blocksize = 10);
x = R*R'; @test norm(as*x*es'+es*x*as'+F*F')/norm(x)/norm(as) < reltol

F = UpperTriangular(rand(Ty,n,n)+im*rand(Ty,n,n))
R = copy(F)
@time plyapcs!(acs,ecs,R,adj = false,blocksize = 10);
x = R*R'; @test norm(acs*x*ecs'+ecs*x*acs'+F*F')/norm(x)/norm(acs) < reltol

F = UpperTriangular(rand(Ty,n,n)+im*rand(Ty,n,n))
R = copy(F)
@time plyapcs!(acs,ecs,R,adj = true,blocksize = 10);
x = R'*R; @test norm(acs'*x*ecs+ecs'*x*acs+F'*F)/norm(x)/norm(acs) < reltol
end
end
